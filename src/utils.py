...
# ---------- Regex patterns for transformer cleaning ----------
_URL_PATTERN = r"http\S+|www\S+"
_TAG_PATTERN = r"\[.*?\]"          # [hook], [quote], [img]...
_HTML_PATTERN = r"<.*?>"
...
def clean_text_transformer(text: str) -> str:
    ...
    # 3. Loại bỏ mention
    text = re.sub(_MENTION_PATTERN, "", text)

    # 4. Xử lý hashtag: giữ phần từ, bỏ #
    text = re.sub(_HASHTAG_PATTERN, r'\1', text)

    # 5. Loại bỏ URL, tag, HTML
    text = re.sub(_URL_PATTERN, "", text)
    text = re.sub(_TAG_PATTERN, "", text)
    text = re.sub(_HTML_PATTERN, "", text)
