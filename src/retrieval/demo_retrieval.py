...
def load_news_corpus(corpus_path=None, max_samples=60000):
    ...
    try:
        if isinstance(corpus_path, str) and (corpus_path.startswith("http://") or corpus_path.startswith("https://")):
            # Tải từ URL
            response = requests.get(corpus_path, timeout=15)
            response.raise_for_status()
            import io
        ...
        # Tự động nhận diện cột văn bản
        if "text" in df.columns:
            corpus = df["text"].fillna("").astype(str).tolist()
        elif "desc" in df.columns:
            title_col = "title" if "title" in df.columns else ""
            if title_col:
                corpus = (df[title_col].fillna("") + " " + df["desc"].fillna("")).astype(str).tolist()
            else:
                corpus = df["desc"].fillna("").astype(str).tolist()
        else:
            if len(df.columns) >= 3:
                # Class, Title, Description
                df.columns = ["class", "title", "desc"]
                corpus = (df["title"].fillna("") + " " + df["desc"].fillna("")).astype(str).tolist()
            else:
                corpus = df.iloc[:, -1].fillna("").astype(str).tolist()

        if max_samples and len(corpus) > max_samples:
            import random
            random.seed(42)
            if len(corpus) > max_samples:
                corpus = random.sample(corpus, max_samples)

        corpus = [t.strip() for t in corpus if t.strip()]
        print(f" Loaded {len(corpus)} documents (using pandas)")
        return corpus
    except Exception as e:
