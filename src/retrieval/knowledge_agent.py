...
def query_wikipedia(entity: str, lang: str = "vi", fetch_full: bool = False) -> str:
    """
    Truy vấn nội dung trực tiếp từ Wikipedia API (không dùng thư viện wikipedia).
    Sử dụng action=opensearch để tìm tiêu đề chính xác và action=query để lấy extract.
    """
    url = f"https://{lang}.wikipedia.org/w/api.php"
    headers = {
        "User-Agent": "FakeNewsDetectionBot/1.0 (contact@fakenewsdetection.com) requests"
    }
    try:
        # 1. Tìm kiếm thực thể (Prefix matching / Search title)
        search_params = {
            "action": "opensearch",
            "format": "json",
            "search": entity,
        ...
        # 2. Lấy tóm tắt / định nghĩa cho tiêu đề đó
        query_params = {
            "action": "query",
            "format": "json",
            "prop": "extracts",
            "exintro": not fetch_full,
            "explaintext": True,
            "titles": title,
            "redirects": 1
        }
        r = requests.get(url, params=query_params, headers=headers, timeout=10)
        if r.status_code != 200:
            return "Not found"
